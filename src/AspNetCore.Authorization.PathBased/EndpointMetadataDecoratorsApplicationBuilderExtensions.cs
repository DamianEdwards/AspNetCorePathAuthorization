...
public static class EndpointMetadataDecoratorsApplicationBuilderExtensions
{
    ...
    public static IEndpointConventionBuilder MapMetadata<T>(this T builder, string pattern, params object[] items)
        where T : IEndpointRouteBuilder
    {
        var _ = builder.ServiceProvider.GetRequiredService<MatcherPolicyMetadata>();

        return builder.Map(pattern, (ctx) => { return Task.CompletedTask; })
